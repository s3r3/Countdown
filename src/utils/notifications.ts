 ...
 * @param eventId - ID of the event for data.
 */
export const scheduleDoneNotification = async (eventName: string, eventId: string) => {
  const { t } = useTranslation();
  await Notifications.scheduleNotificationAsync({
    content: {
...
export const setupNotificationListener = () => {
  ...
  const subscription = Notifications.addNotificationResponseReceivedListener((response) => {
    const { eventId, action } = response.notification.request.content.data as { eventId: string; action: string };
